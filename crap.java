...
public class TileGame{
	...
	if(gameBoard.size() == 0){
		...
		for(int i = 0; i <= gameBoard.size(); i++){
			x = i;
			if((x == 0)&&(gameBoard.size()==1)){
				NumberTile a = (NumberTile)gameBoard.get(i);
				if(a.getLeft() == nt.getRight()){
					ret = 0;
				}
				if(a.getRight() == nt.getLeft()){
					ret = 1;
				}
			}
		}
		else if(x == gameBoard.size()){
			NumberTile a = (NumberTile)gameBoard.get(x);
				if(a.getRight() == nt.getLeft()){
					ret = gameBoard.size();
				}
			}
			...
			NumberTile a = (NumberTile)gameBoard.get(x);
			NumberTile b = (NumberTile)gameBoard.get(x+1);
			if((a.getRight() == nt.getLeft())&&(b.getLeft() == nt.getRight())){
			ret = x + 1;
			}	
		}
	}
